...
impl<B: Backend> TabsApp<B> {
    pub fn new() -> Self {
        let temperatures = [62, 65, 72, 78, 75, 68];
        Self {
            selected_tab: 0,
            temperatures,
            _marker: PhantomData,
    ...
    pub fn run(&mut self, terminal: &mut Terminal<B>, button: &mut Button<'_>, delay: &Delay)
    where
        B::Error: 'static,
    {
        log::debug!("Running the Tabs app");
        loop {
            if button.was_pressed() {
                ...
                self.selected_tab += 1;
                log::trace!("Switching to the next tab");
            }
            terminal
    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        match self.selected_tab {
            0 => {
                ...
                render_footer("[Space] to change tab", footer_area, buf);
            }
            1 => {
                let default_style = Style::default()
                    .bg(Color::Rgb(50, 50, 50))
                    .fg(Color::Yellow);
                let list = CalendarEventStore::default();
                Monthly::new(
            ...
            2 => {
                vertical_barchart(&self.temperatures)
                    .block(block)
                    .render(inner_area, buf);
                render_footer("[Space] to change screen", footer_area, buf);
            }
            _ => {}
        }
    }
