...
use crate::helpers::{Delay, center};
...
impl<B: Backend> ChartApp<B> {
    ...
    where
        B::Error: 'static,
    {
        log::debug!("Running the Chart app");
        loop {
    ...
    fn draw(&mut self, frame: &mut Frame) {
        ...
        if self.popup {
            let footer = Line::raw("[Space] to change screen").centered().gray();
            ...
            let footer = Line::raw("[Space] to display popup").centered().gray();
            frame.render_widget(footer, footer_area);
        }
    }
}
