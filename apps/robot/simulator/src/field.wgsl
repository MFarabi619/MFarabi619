...
const ROAD_HALF_WIDTH: f32 = 0.30;
...
fn terrain_height(world: vec2<f32>) -> f32 {
    if (abs(world.y) < ROAD_HALF_WIDTH) {
        return 0.0;
    }
...
fn surface_color(p: vec3<f32>, view: vec3<f32>) -> vec3<f32> {
    ...
    let field_ground = mix(dirt, tarp, tarp_cover);
    let on_road = 1.0 - smoothstep(ROAD_HALF_WIDTH - 0.04, ROAD_HALF_WIDTH, abs(world.y));
    let on_line = 1.0 - smoothstep(LINE_HALF_WIDTH - 0.015, LINE_HALF_WIDTH, abs(world.y));
    let road = mix(dirt, vec3<f32>(0.92, 0.92, 0.90), on_line);
    let ground = mix(field_ground, road, on_road);
