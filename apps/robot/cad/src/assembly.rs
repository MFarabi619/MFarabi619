...
fn camera_links() -> Result<Vec<Link>, Box<dyn Error>> {
    let origin = placement::camera_origin();
    let solids = place_component(
    ...
    Ok(vec![
        Link {
            id: LinkId::CameraLink,
            origin_world: origin,
            solids,
        },
        Link {
            id: LinkId::CameraOptical,
            origin_world: origin,
            solids: Vec::new(),
        },
    ])
...
pub fn drive_train() -> Result<Vec<Link>, Box<dyn Error>> {
...
pub fn robot() -> Result<Vec<Link>, Box<dyn Error>> {
    ...
    let drive_train_links = time_it!("drive_train", drive_train())?;
    let camera = time_it!("camera", camera_links())?;
    ...
    links.extend(drive_train_links);
    links.extend(camera);
    Ok(links)
}
