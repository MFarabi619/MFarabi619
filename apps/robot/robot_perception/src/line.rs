...
pub(crate) fn point(x: f64, y: f64) -> Point2 {
    Point2 { x, y }
}
...
fn follow(detection: &LineDetection, forward_speed: f64, steering_gain: f64) -> TwistStamped {
    if detection.found {
...
pub async fn run_line_follower(
    node: Arc<Node>,
    target_color: LineColor,
    start_enabled: bool,
) -> Result<(), BoxError> {
    ...
    tracing::info!(
        "line follower on {} -> cmd_vel + /line/overlay (forward {forward_speed} m/s)",
