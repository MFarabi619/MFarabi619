...
pub fn drivetrain_params(config: &crate::config::RobotConfig, dimensions: &Dimensions) -> String {
    use robot_description::placement::{wheel_origin, Corner};
    let wheel_radius = WHEEL_TREAD_DIAMETER_MM / 2.0 * MM_TO_M;
    let wheel_separation = config
        .platform
        .drivetrain
        .wheel_separation_m
        .unwrap_or_else(|| 2.0 * wheel_origin(Corner::RearLeft, dimensions).y.abs() * MM_TO_M);
    format!(
...
pub fn write(
    ...
    for link in links {
        ...
        if !link.solids.is_empty() {
            ...
            body.push_str(&format!(
                "    <visual><geometry><mesh filename=\"{MESH_URI_PREFIX}{robot_name}/{mesh_filename}\"/></geometry></visual>\n",
            ));
        }
        if let Some(collision) = collision_xml(link.id, dimensions) {
...
fn collision_xml(id: LinkId, dimensions: &Dimensions) -> Option<String> {
    match id {
        ...
        LinkId::CasterFrontLeft | LinkId::CasterFrontRight => Some(format!(
