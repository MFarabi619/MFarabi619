...
fn subtract_cutters(shape: &Solid, cutters: &[Solid]) -> Result<Solid, cadrum::Error> {
    cutters
        .iter()
        .fold(Boolean::from(shape), |difference, cutter| difference - cutter)
        .build()
...
fn frame(dimensions: &Dimensions) -> Result<Vec<Solid>, cadrum::Error> {
    ...
    Ok([
        side_rail
            .clone()
            .translate(DVec3::new(0.0, side_rail_center_y, dimensions.frame_top_z_mm)),
        side_rail.translate(DVec3::new(0.0, -side_rail_center_y, dimensions.frame_top_z_mm)),
        cross_rail
            .clone()
...
fn outer_face_gusset() -> Result<Solid, cadrum::Error> {
    ...
    cutters.extend(hole_row(
        hole_radius,
        cutter_axis,
        DVec3::new(GUSSET_HOLE_PITCH_MM, cutter_y_start, -RAIL_CROSS_HEIGHT_MM / 2.0),
    ...
    for post_x_sign in [1.0, -1.0] {
        for post_y_sign in [1.0, -1.0] {
            ...
            for x_side_sign in [1.0, -1.0] {
                ...
                gussets.push(
                    gusset
                        .translate(DVec3::new(post_x, post_outer_face_y, dimensions.frame_top_z_mm))
...
fn crossover_connectors(dimensions: &Dimensions) -> Result<Vec<Solid>, Box<dyn Error>> {
    let imported = Solid::read_step(&mut File::open(asset(CROSSOVER_CONNECTOR_STEP))?)?;
...
fn recolored_e_stop_button() -> Result<Vec<Solid>, Box<dyn Error>> {
    let imported = Solid::read_step(&mut File::open(asset(E_STOP_BUTTON_STEP))?)?;
    Ok(imported
        .into_iter()
        .map(|mut solid| {
...
fn side_mounted_enclosure(
    ...
    let imported = Solid::read_step(&mut File::open(asset(asset_name))?)?;
    let oriented: Vec<Solid> = imported
        .into_iter()
...
fn orbbec_gemini_335l_on_ball_head_mount(
    ...
    let imported = Solid::read_step(&mut File::open(asset(BALL_HEAD_MOUNT_STEP))?)?;
    let oriented: Vec<Solid> = imported
        .into_iter()
        .map(|solid| solid.rotate_y(FRAC_PI_2).with_material(Material::BlackPlastic))
        .collect();
    ...
    let camera_imported = Solid::read_step(&mut File::open(asset(ORBBEC_GEMINI_335L_STEP))?)?;
...
fn caster_mount_bars(dimensions: &Dimensions) -> Result<Vec<Solid>, Box<dyn Error>> {
    let imported = Solid::read_step(&mut File::open(asset(PRE_DRILLED_1X1_ALUMINUM_BAR_STEP))?)?;
...
fn casters(dimensions: &Dimensions) -> Result<Vec<Solid>, Box<dyn Error>> {
    let imported = Solid::read_step(&mut File::open(asset(CASTER_STEP))?)?;
    let oriented: Vec<Solid> = imported
        .into_iter()
        .map(|solid| {
...
pub fn robot(dimensions: &Dimensions) -> Result<Vec<Link>, Box<dyn Error>> {
    ...
    chassis_solids.extend(time_it!("caster_mount_bars", caster_mount_bars(dimensions))?);
