...
MACHINES_PATH = Path(__file__).resolve().parents[2] / 'machines'
SAMPLES = sorted(MACHINES_PATH.glob('*/robot.yaml'))


def test_samples_are_present():
    assert SAMPLES, f'no robot.yaml samples under {MACHINES_PATH}'
...
def test_robot_yaml_parses(sample):
    ...
    assert str(config.serial_number).startswith('mark-')
