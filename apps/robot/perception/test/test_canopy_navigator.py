...
def test_centered_row_drives_straight(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(projected_mask([(0.0, 0.0)]))
    assert len(commands) == 1
    assert commands[0].twist.linear.x == pytest.approx(0.4)
    assert commands[0].twist.angular.z == pytest.approx(0.0, abs=0.06)


def test_row_left_of_robot_steers_left(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(projected_mask([(0.15, 0.0)]))
    assert commands[0].twist.angular.z > 0.1


def test_row_right_of_robot_steers_right(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(projected_mask([(-0.15, 0.0)]))
    assert commands[0].twist.angular.z < -0.1


def test_heading_error_steers_to_realign(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(projected_mask([(0.0, 0.08)]))
    assert commands[0].twist.angular.z > 0.02


def test_offset_label_reports_meters(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(projected_mask([(0.15, 0.0)]))
    assert reported_lateral_error(overlays[0]) == pytest.approx(-0.15, abs=0.05)


def test_full_lattice_of_rows_agrees_on_center(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(
        projected_mask([(-1.2, 0.0), (0.0, 0.0), (1.2, 0.0)]))
    assert commands[0].twist.angular.z == pytest.approx(0.0, abs=0.06)


def test_offset_lattice_steers_back(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(
...
def test_overlay_draws_corridor_grid(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(projected_mask([(0.0, 0.0)]))
...
def test_corridor_center_overlays_the_lock_line(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(projected_mask([(0.15, 0.0)]))
...
def test_bare_mask_holds_position(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(projected_mask([]))
    assert commands == []
...
def test_elevated_canopy_fools_flat_ground_fallback(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    mask, depth = elevated_canopy([(0.0, 0.0)], 0.15)
    canopy_navigator_node.on_mask(mask)
    assert abs(commands[0].twist.angular.z) > 0.1


def test_depth_makes_elevated_canopy_exact(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    mask, depth = elevated_canopy([(0.0, 0.0)], 0.15)
    canopy_navigator_node.on_depth(depth)
    canopy_navigator_node.on_mask(mask)
    assert commands[0].twist.angular.z == pytest.approx(0.0, abs=0.06)


def test_depth_recovers_offset_of_elevated_canopy(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
...
def test_goal_drives_and_streams_feedback(canopy_navigator_node):
    canopy_navigator_node.drive_enabled = False
    commands, overlays = capture(canopy_navigator_node)
    handle = begin_goal(canopy_navigator_node)
    canopy_navigator_node.on_mask(projected_mask([(0.0, 0.0)]))
    assert len(commands) == 1
...
def test_goal_speed_overrides_parameter(canopy_navigator_node):
    canopy_navigator_node.drive_enabled = False
    commands, overlays = capture(canopy_navigator_node)
    begin_goal(canopy_navigator_node, forward_speed_mps=0.8)
    canopy_navigator_node.on_mask(projected_mask([(0.0, 0.0)]))
    assert commands[0].twist.linear.x == pytest.approx(0.8)


def test_odometry_reaches_goal_distance(canopy_navigator_node):
    begin_goal(canopy_navigator_node, distance_m=2.0)
    canopy_navigator_node.on_odom(odometry(0.0, 0.0))
    canopy_navigator_node.on_odom(odometry(1.2, 0.0))
    assert canopy_navigator_node.goal_outcome is None
    canopy_navigator_node.on_odom(odometry(2.1, 0.0))
    assert canopy_navigator_node.goal_outcome == 'distance_reached'
    assert canopy_navigator_node.distance_traveled_m == pytest.approx(2.1)


def test_losing_the_row_aborts_the_goal(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    begin_goal(canopy_navigator_node, distance_m=10.0)
    for _ in range(12):
        canopy_navigator_node.on_mask(projected_mask([]))
    assert canopy_navigator_node.goal_outcome == 'row_lost'


def test_end_of_row_completes_untargeted_goal(canopy_navigator_node):
    handle = begin_goal(canopy_navigator_node)
    canopy_navigator_node.distance_traveled_m = 5.0
    canopy_navigator_node.conclude(handle, 'row_lost')
    assert handle.status == 'succeeded'


def test_row_lost_fails_distance_goal(canopy_navigator_node):
    handle = begin_goal(canopy_navigator_node, distance_m=10.0)
    canopy_navigator_node.conclude(handle, 'row_lost')
    assert handle.status == 'aborted'


def test_phantom_row_lost_before_minimum_travel_fails(canopy_navigator_node):
    handle = begin_goal(canopy_navigator_node)
    canopy_navigator_node.distance_traveled_m = 1.0
    canopy_navigator_node.conclude(handle, 'row_lost')
    assert handle.status == 'aborted'
...
def test_scattered_real_canopy_still_locks(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(scattered_canopy_mask(0.15))
    assert len(commands) == 1
    assert reported_lateral_error(overlays[0]) == pytest.approx(-0.15, abs=0.07)


def test_row_starting_ahead_of_robot_still_locks(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(
...
def test_grass_speckle_is_not_a_row(canopy_navigator_node):
    commands, overlays = capture(canopy_navigator_node)
    canopy_navigator_node.on_mask(speckle_mask(0.25))
    assert commands == []
    assert overlays[0].points == []


def test_without_camera_info_stays_silent(canopy_navigator_node):
    commands = []
    overlays = []
    canopy_navigator_node.cmd_vel_publisher.publish = commands.append
    canopy_navigator_node.overlay_publisher.publish = overlays.append
    canopy_navigator_node.on_mask(projected_mask([(0.0, 0.0)]))
    assert commands == []
    assert overlays == []
