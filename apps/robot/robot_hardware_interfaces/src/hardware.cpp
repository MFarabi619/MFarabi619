...
namespace {
constexpr int kPwmPhaseOffset = 0;
constexpr int kPwmCyclesContinuous = 0;
constexpr double kMaxDutyPercent = 100.0;
} // namespace
...
RobotHardware::write(const rclcpp::Time & /*time*/,
                     const rclcpp::Duration & /*period*/) {
  drive_side(left_pwm_pin_, left_dir_pin_, left_forward_level_,
             hw_commands_velocity_[left_index_]);
  drive_side(right_pwm_pin_, right_dir_pin_, right_forward_level_,
             hw_commands_velocity_[right_index_]);
  return hardware_interface::return_type::OK;
}

void RobotHardware::drive_side(int pwm_pin, int dir_pin, bool forward_level,
                               double wheel_speed) {
  gpio_write(sbc_, chip_, dir_pin,
             ((wheel_speed >= 0.0) == forward_level) ? 1 : 0);
  const double duty =
      std::clamp(std::abs(wheel_speed) / max_wheel_speed_ * kMaxDutyPercent,
                 0.0, kMaxDutyPercent);
  tx_pwm(sbc_, chip_, pwm_pin, pwm_frequency_, static_cast<float>(duty),
         kPwmPhaseOffset, kPwmCyclesContinuous);
}

void RobotHardware::halt() {
  // Stop by driving 0% duty at a valid frequency (holds the pin low). tx_pwm
  // with frequency 0 returns LG_BAD_PWM_MICROS (-86) on a fresh pin and does
  // NOT stop.
  tx_pwm(sbc_, chip_, left_pwm_pin_, pwm_frequency_, 0.0f, kPwmPhaseOffset,
         kPwmCyclesContinuous);
  tx_pwm(sbc_, chip_, right_pwm_pin_, pwm_frequency_, 0.0f, kPwmPhaseOffset,
         kPwmCyclesContinuous);
}
