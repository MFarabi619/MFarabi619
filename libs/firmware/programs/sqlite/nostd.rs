...
pub use super::capi::*;
use super::bindings;
use super::capi as sqlite3_capi;
...
mod tests {
    ...
    extern "C" fn rust_test_sqlite_engine_announces_version(_path: *const c_char) -> c_int {
        given("the SQLite engine is linked in");
        when("sqlite3_libversion is read");
        let version = unsafe { CStr::from_ptr(super::super::bindings::sqlite3_libversion()) };
        if !version.to_bytes().starts_with(b"3.") {
            return -1;
        }
        then("the version string starts with the major version 3");
        0
    }

    #[no_mangle]
    extern "C" fn rust_test_sqlite_opens_and_closes_cleanly(path: *const c_char) -> c_int {
        given("an unused database path on littlefs");
        when("open is called");
        let database = match super::open(path) {
            Ok(database) => database,
            Err(_) => return -1,
        };
        then("a managed connection is returned");
        and("dropping the connection closes it without error");
        drop(database);
        0
    }

    #[no_mangle]
    extern "C" fn rust_test_sqlite_inserts_and_queries_back_one_row(path: *const c_char) -> c_int {
        given("a fresh database with a single-column table");
        let database = match super::open(path) {
            Ok(database) => database,
            Err(_) => return -1,
        };
        if database.exec_safe("CREATE TABLE t(v TEXT)").is_err() {
            return -1;
        }
        when("a row is inserted and queried back via prepare/step/column");
        if database.exec_safe("INSERT INTO t(v) VALUES('hello')").is_err() {
            return -1;
        }
        let statement = match database.prepare_v2("SELECT v FROM t LIMIT 1") {
            Ok(statement) => statement,
            Err(_) => return -1,
        };
        if statement.step() != Ok(ResultCode::ROW) {
            return -1;
        }
        let value = match statement.column_text(0) {
            Ok(value) => value,
            Err(_) => return -1,
        };
        then("the queried text matches what was inserted");
        if value != "hello" {
            return -1;
        }
        0
    ...
    #[no_mangle]
    extern "C" fn rust_test_sqlite_prepared_statement_binds_and_steps(path: *const c_char) -> c_int {
        given("a prepared INSERT statement with one bind parameter");
        let database = match super::open(path) {
            Ok(database) => database,
            Err(_) => return -1,
        };
        ...
        let statement = match database.prepare_v2("INSERT INTO t(v) VALUES(?)") {
            Ok(statement) => statement,
            Err(_) => return -1,
        };
        when("two different values are bound and stepped in sequence");
        for value in ["alpha", "beta"] {
            if statement
                .bind_text(1, value, super::Destructor::TRANSIENT)
                .is_err()
            {
                return -1;
            }
            if statement.step() != Ok(ResultCode::DONE) {
                return -1;
            }
            if statement.reset().is_err() {
                return -1;
            }
        }
        then("the table contains exactly two rows");
        let count = match database.prepare_v2("SELECT count(*) FROM t") {
            Ok(count) => count,
            Err(_) => return -1,
        };
        if count.step() != Ok(ResultCode::ROW) {
            return -1;
        }
        if count.column_int(0) != 2 {
            return -1;
        }
        0
    }
    ...
    #[no_mangle]
    extern "C" fn rust_test_sqlite_transaction_commit_persists(path: *const c_char) -> c_int {
        given("a row inserted inside BEGIN/COMMIT");
        {
            ...
            if database.exec_safe("BEGIN").is_err() {
                return -1;
            }
            if database.exec_safe("INSERT INTO t(v) VALUES('committed')").is_err() {
                return -1;
            }
            if database.exec_safe("COMMIT").is_err() {
                return -1;
            }
        }
        ...
        then("the committed row is still readable");
        let statement = match database.prepare_v2("SELECT v FROM t") {
            Ok(statement) => statement,
            Err(_) => return -1,
        };
        if statement.step() != Ok(ResultCode::ROW) {
        ...
        if statement.column_text(0) != Ok("committed") {
            return -1;
        }
        0
    }
    ...
    #[no_mangle]
    extern "C" fn rust_test_sqlite_transaction_rollback_discards(path: *const c_char) -> c_int {
        given("three rows inserted inside BEGIN and then ROLLBACK");
        let database = match super::open(path) {
            Ok(database) => database,
            Err(_) => return -1,
        };
        ...
        for value in ["a", "b", "c"] {
            let sql = alloc::format!("INSERT INTO t(v) VALUES('{value}')");
            if database.exec_safe(&sql).is_err() {
                return -1;
            }
        }
        if database.exec_safe("ROLLBACK").is_err() {
            return -1;
        }
        when("the table is queried for its row count");
        let statement = match database.prepare_v2("SELECT count(*) FROM t") {
            Ok(statement) => statement,
            Err(_) => return -1,
        };
        if statement.step() != Ok(ResultCode::ROW) {
    ...
    #[no_mangle]
    extern "C" fn rust_test_sqlite_invalid_sql_returns_error(path: *const c_char) -> c_int {
        given("an open database");
        let database = match super::open(path) {
            Ok(database) => database,
            Err(_) => return -1,
        };
        when("a syntactically invalid statement is executed");
        let result = database.exec_safe("NOT VALID SQL");
        then("exec_safe returns an Err rather than Ok");
        if result.is_ok() {
            return -1;
        }
        0
    ...
    #[no_mangle]
    extern "C" fn rust_test_sqlite_data_persists_across_reopen(path: *const c_char) -> c_int {
        given("a row persisted to littlefs by closing the connection");
        {
            let database = match super::open(path) {
                Ok(database) => database,
                Err(_) => return -1,
            ...
            if database.exec_safe("INSERT INTO t(v) VALUES('survive')").is_err() {
                return -1;
            }
        }
        when("the database is reopened from the same path");
        let database = match super::open(path) {
            Ok(database) => database,
            Err(_) => return -1,
        };
        ...
        then("the surviving row reads back identically");
        if statement.column_text(0) != Ok("survive") {
            return -1;
        }
        0
    }
    ...
    #[no_mangle]
    extern "C" fn rust_test_sqlite_empty_table_step_returns_done(path: *const c_char) -> c_int {
        ...
        when("SELECT is stepped on the empty table");
        let statement = match database.prepare_v2("SELECT v FROM t") {
            Ok(statement) => statement,
            Err(_) => return -1,
        };
        let outcome = statement.step();
        then("the first step returns DONE immediately, not ROW");
        if outcome != Ok(ResultCode::DONE) {
            return -1;
        }
        0
    }
