...
2. Install [[https://determinate.systems][Determinate Nix]]
    #+begin_src sh
    curl -fsSL https://install.determinate.systems/nix | sh -s -- install --no-confirm --extra-conf "trusted-users = root $USER"
    #+end_src

   # Install [[https://lix.systems][Lix]], a fork of [[https://determinate.systems][Determinate Nix]]
    #+begin_src sh
...
Obtain [[https://guix.gnu.org/en/download/][ISO]].
...
hint: Consider setting the necessary environment variables by running:
     ...
     GUIX_PROFILE="/home/mfarabi/.config/guix/current"
     . "$GUIX_PROFILE/etc/profile"
     unset GUIX_PROFILE

Alternately, see `guix package --search-paths -p "/home/mfarabi/.config/guix/current"'.


hint: After setting `PATH', run `hash guix' to make sure your shell refers to
`/home/mfarabi/.config/guix/current/bin/guix'.

** [[https://www.openbsd.org][OpenBSD]]

[[https://openbsd.app][OpenBSD.app - search packages]]

Obtain [[https://www.openbsd.org/faq/faq4.html#Download][ISO]].

#+begin_src ksh
su -
echo 'permit persist mfarabi' > /etc/doas.conf
...
#+begin_src ksh
doas pkg_add gnupg pinentry sops bash zsh git delta direnv ripgrep eza fd vim-9.1.1706-gtk3 neovim emacs-30.2-pgtk xdg-{user-dirs,dbus-proxy,desktop-portal,utils} libqtxdg consolekit2 ruby34-xdg
#+end_src

Set up zsh.

#+begin_src zsh
doas pkg_add rust rust-{analyzer,rustfmt,clippy,gdb,src,bindgen}
#+end_src

#+begin_src zsh
doas pkg_add nix curl bat jetbrains-mono noto-{emoji,fonts,nerd-fonts} symbola-ttf libvips lf
#+end_src
...
#+begin_src zsh
doas pkg_add github-cli tailscale ttyd
#+end_src
...
#+begin_src zsh
doas pkg_add btop fastfetch cmatrix cowsay asciiquarium figlet nyancat
#+end_src

#+begin_src zsh
doas pkg_add termshark
#+end_src

#+begin_src zsh
doas pkg_add tree fzf cmake pandoc zig go gopls uv python-3.12.11 node ruby-3.4.6 gnuplot-5.4.8 graphviz tidy maim scrot clang-tools-extra stylua luarocks-lua51 wget socat tree-sitter shfmt ffmpeg ffmpegthumbnailer coreutils unzip-6.0p18-iconv p7zip aspell llvm-21.1.2p0 mu gnutls
#+end_src

#+begin_src zsh
doas pkg_add xclip xsel dbus kitty vlc dunst mpv rofi dolphin cairo pipewire slurp wayland-{utils,protocols} xwayland wlr-randr pango xloadimage libsixel jq poppler poppler-utils ImageMagick gcc-11.2.0p19 gcc-libs-11.2.0p19
#+end_src

#+begin_src zsh
doas pkg_add bwm-ng sshfs-fuse
#+end_src

#+begin_src zsh
doas pkg_add ncdu stress smartmontools gnupg {sys,bat}mon netpbm sqlite3
#+end_src
...
#+begin_src sh
sudo systemctl stop pvestatd pvedaemon pve-cluster corosync

sudo rm -f /var/lib/pve-cluster/.pmxcfs.lockfile
sudo rm -rf /var/lib/pve-cluster
sudo rm /etc/pve/corosync.conf
sudo rm /etc/corosync/*
sudo rm /var/lib/corosync/*
#+end_src


sudo systemctl start pvestatd pvedaemon pve-cluster corosync

#+begin_src sh
sudo pvecm create openws
sudo pvecm status
#+end_src
...
#+begin_src bash
ln -nfs /home/mfarabi/MFarabi619/configurations/home/mfarabi@archlinux/.bashrc ~/.bashrc;
#+end_src
...
#+begin_src bash
sudo apt install greetd tuigreet dconf-service
#+end_src
...
2. Remove bloat
  ...
  Open Windows Terminal as Admin with SUPER+X, A.
...
#+begin_src powershell
    Disable-WindowsOptionalFeature -Online -NoRestart -FeatureName @(
        "WindowsMediaPlayer",
