...
#+begin_src ksh
doas pkg_add bash zsh git delta direnv ripgrep eza fd vim-9.1.1706-gtk3 neovim emacs-30.2-gtk3
#+end_src

Set up zsh.

#+begin_src zsh
doas pkg_add rust rust-{rustfmt,clippy,gdb,src,bindgen}
#+end_src

#+begin_src zsh
doas pkg_add nix curl bat jetbrains-mono noto-{emoji,fonts,nerd-fonts} symbolsonly-nerd-fonts libvips lf
#+end_src
...
#+begin_src zsh
doas pkg_add tree fzf cmake pandoc zig go gopls uv stylua luarocks-lua51 wget socat tree-sitter shfmt ffmpeg ffmpegthumbnailer coreutils unzip-6.0p18-iconv p7zip aspell llvm-21.1.2p0
#+end_src

#+begin_src zsh
doas pkg_add xclip dbus kitty vlc dunst mpv rofi dolphin cairo pipewire slurp wayland-{utils,protocols} xwayland wlr-randr pango xloadimage libsixel

jq poppler poppler-utils ImageMagick  xsel gcc-11.2.0p19
#+end_src
