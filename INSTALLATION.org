...
#+begin_src bash
pkg install git bash zsh bat lazygit lf terminfo-db dunst mpv direnv zellij markdown jetbrains-mono nerd-fonts noto-emoji npm vips docker docker-compose k9s
#+end_src
...
5. Modify in =/etc/ssh/sshd_config=
#+begin_src bash
