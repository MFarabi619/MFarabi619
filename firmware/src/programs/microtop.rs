...
use crate::config;
use crate::config::topology::CURRENT_TOPOLOGY;
...
impl AnsiBackend {
    ...
    fn ansi_fg(&mut self, color: Color) {
        match color {
            Color::Reset => { let _ = write!(self.buf, "\x1b[39m"); }
            Color::Black => { let _ = write!(self.buf, "\x1b[30m"); }
            Color::Red => { let _ = write!(self.buf, "\x1b[31m"); }
            Color::Green => { let _ = write!(self.buf, "\x1b[32m"); }
            Color::Yellow => { let _ = write!(self.buf, "\x1b[33m"); }
            Color::Blue => { let _ = write!(self.buf, "\x1b[34m"); }
            Color::Magenta => { let _ = write!(self.buf, "\x1b[35m"); }
            Color::Cyan => { let _ = write!(self.buf, "\x1b[36m"); }
            Color::White => { let _ = write!(self.buf, "\x1b[37m"); }
            Color::Gray => { let _ = write!(self.buf, "\x1b[90m"); }
            Color::DarkGray => { let _ = write!(self.buf, "\x1b[90m"); }
            Color::LightRed => { let _ = write!(self.buf, "\x1b[91m"); }
            Color::LightGreen => { let _ = write!(self.buf, "\x1b[92m"); }
            Color::LightYellow => { let _ = write!(self.buf, "\x1b[93m"); }
            Color::LightBlue => { let _ = write!(self.buf, "\x1b[94m"); }
            Color::LightMagenta => { let _ = write!(self.buf, "\x1b[95m"); }
            Color::LightCyan => { let _ = write!(self.buf, "\x1b[96m"); }
            Color::Indexed(i) => { let _ = write!(self.buf, "\x1b[38;5;{}m", i); }
            Color::Rgb(r, g, b) => { let _ = write!(self.buf, "\x1b[38;2;{};{};{}m", r, g, b); }
        }
    }

    fn ansi_bg(&mut self, color: Color) {
        match color {
            Color::Reset => { let _ = write!(self.buf, "\x1b[49m"); }
            Color::Black => { let _ = write!(self.buf, "\x1b[40m"); }
            Color::Red => { let _ = write!(self.buf, "\x1b[41m"); }
            Color::Green => { let _ = write!(self.buf, "\x1b[42m"); }
            Color::Yellow => { let _ = write!(self.buf, "\x1b[43m"); }
            Color::Blue => { let _ = write!(self.buf, "\x1b[44m"); }
            Color::Magenta => { let _ = write!(self.buf, "\x1b[45m"); }
            Color::Cyan => { let _ = write!(self.buf, "\x1b[46m"); }
            Color::White => { let _ = write!(self.buf, "\x1b[47m"); }
            Color::Gray | Color::DarkGray => { let _ = write!(self.buf, "\x1b[100m"); }
            Color::LightRed => { let _ = write!(self.buf, "\x1b[101m"); }
            Color::LightGreen => { let _ = write!(self.buf, "\x1b[102m"); }
            Color::LightYellow => { let _ = write!(self.buf, "\x1b[103m"); }
            Color::LightBlue => { let _ = write!(self.buf, "\x1b[104m"); }
            Color::LightMagenta => { let _ = write!(self.buf, "\x1b[105m"); }
            Color::LightCyan => { let _ = write!(self.buf, "\x1b[106m"); }
            Color::Indexed(i) => { let _ = write!(self.buf, "\x1b[48;5;{}m", i); }
            Color::Rgb(r, g, b) => { let _ = write!(self.buf, "\x1b[48;2;{};{};{}m", r, g, b); }
        }
    }
}
...
pub fn render_frame(width: u16, height: u16) -> AllocString {
    ...
    let _ = write!(title_str, " {} @ {} | Uptime: {}h {}m {}s ", shell::SSH_USER, config::HOSTNAME, h, m, s);

    let mut cpu_str = AllocString::new();
    let _ = write!(cpu_str, "Xtensa LX7 @ {} MHz", clock::cpu_clock().as_mhz());
    let mut ip_str = AllocString::new();
    let _ = write!(ip_str, "{}.{}.{}.{}", info.ip_address[0], info.ip_address[1], info.ip_address[2], info.ip_address[3]);
    let mut ports_str = AllocString::new();
    let _ = write!(ports_str, "SSH:{} HTTP:{} OTA:{}", config::ssh::PORT, config::http::PORT, config::ota::PORT);
    ...
    if info.sd_card_size_mb > 0 {
        let gb = info.sd_card_size_mb as f32 / 1024.0;
    ...
    if co2.ok {
        let _ = write!(co2_str, "{:.1} ppm", co2.co2_ppm);
        let _ = write!(temp_str, "{:.1}\u{00b0}C", co2.temperature);
        let _ = write!(rh_str, "{:.1}%", co2.humidity);
    }
    ...
    let _ = terminal.draw(|frame| {
        ...
        if co2.ok {
            ...
            let _ = sensor_rows.push(Row::new(["Sensor", co2.model]));
        } else {
            let _ = sensor_rows.push(Row::new(["Status", "No readings"]));
        }

        for sensor in CURRENT_TOPOLOGY.enabled_sensors() {
            let _ = sensor_rows.push(Row::new([sensor.model, sensor.bus_label]));
        }
