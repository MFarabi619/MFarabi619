...
ConsoleClass::ConsoleClass()
  : _initialized(false), _replStarted(false), _replTaskHandle(NULL), _prompt("esp> "), _maxHistory(32), _historyVfsPath(NULL), _taskStackSize(4096),
    _taskPriority(2), _taskCore(tskNO_AFFINITY), _usePsram(true), _taskStackInPsram(false), _forceMode(false), _cmds{}, _cmdCount(0) {}
...
bool ConsoleClass::addCmd(const char *name, const char *help, const char *hint, ConsoleCommandFunc func) {
  ...
  _trackCmd(name, help, hint);
  return true;
}

void ConsoleClass::_trackCmd(const char *name, const char *help, const char *hint) {
  if (_cmdCount < MAX_CMDS) {
    _cmds[_cmdCount++] = {name, hint, help};
  }
}
...
int ConsoleClass::_helpHandler(int argc, char **argv) {
  if (argc == 2) {
    for (size_t i = 0; i < Console._cmdCount; i++) {
      if (strcmp(Console._cmds[i].name, argv[1]) == 0) {
        printf("  %s", Console._cmds[i].name);
        if (Console._cmds[i].hint) printf("  %s", Console._cmds[i].hint);
  ...
  printf("\n");
  for (size_t i = 0; i < Console._cmdCount; i++) {
    printf("  \x1b[32m%-20s\x1b[0m", Console._cmds[i].name);
    if (Console._cmds[i].hint)
      printf(" %-28s", Console._cmds[i].hint);
    else
      printf(" %-28s", "");
    printf(" %s\n", Console._cmds[i].help);
  }
...
bool ConsoleClass::addHelpCmd() {
  return addCmd("help", "show available commands", "[command]", _helpHandler);
}
