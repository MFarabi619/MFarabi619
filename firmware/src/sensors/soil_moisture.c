...
#define SOIL_MOISTURE_REGISTER_START         0
#define SOIL_MOISTURE_REGISTER_COUNT         2
#define SOIL_MOISTURE_INDEX_MOISTURE         0
#define SOIL_MOISTURE_INDEX_TEMPERATURE      1
...
struct soil_moisture_data {
	int iface;
	uint16_t register_values[SOIL_MOISTURE_REGISTER_COUNT];
};

static int soil_moisture_init(const struct device *device)
{
	const struct soil_moisture_config *config = device->config;
	struct soil_moisture_data *data = device->data;
...
static int soil_moisture_sample_fetch(const struct device *device, enum sensor_channel channel)
{
	const struct soil_moisture_config *config = device->config;
	struct soil_moisture_data *data = device->data;
	...
	int read_status = modbus_read_holding_regs(data->iface, config->slave_id,
						   SOIL_MOISTURE_REGISTER_START,
						   data->register_values,
						   SOIL_MOISTURE_REGISTER_COUNT);
	if (read_status != 0) {
		LOG_ERR("Failed to read SEN0600 slave %u: %d",
...
static int soil_moisture_channel_get(const struct device *device, enum sensor_channel channel,
				     struct sensor_value *value)
{
	...
	case SENSOR_CHAN_CERATINA_SOIL_MOISTURE: {
		uint16_t raw_moisture = data->register_values[SOIL_MOISTURE_INDEX_MOISTURE];
	...
	case SENSOR_CHAN_AMBIENT_TEMP: {
		int16_t raw_temperature =
			(int16_t)data->register_values[SOIL_MOISTURE_INDEX_TEMPERATURE];
...
static int soil_moisture_attr_get(const struct device *device, enum sensor_channel channel,
