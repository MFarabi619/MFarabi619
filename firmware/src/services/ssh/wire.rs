use super::codec::{ObjectReader, ObjectWriter};
...
impl<'a> Request<'a> {
    pub fn decode_with(reader: &mut ObjectReader<'a>) -> Result<Self, ProtocolError> {
        let decoded = match reader.read_string_utf8()? {
            ...
            "pty-req" => {
                ...
                let term = reader.read_string_utf8()?;
                let width_chars = reader.read_uint32()?;
                let height_rows = reader.read_uint32()?;
                let width_pixels = reader.read_uint32()?;
                let height_pixels = reader.read_uint32()?;
                // Consume remaining bytes (encoded terminal modes)
                let _ = reader.read_remaining();
                Self::PtyReq { want_reply, term, width_chars, height_rows, width_pixels, height_pixels }
            }
    ...
    pub fn encode_with(self, writer: &mut ObjectWriter) -> Result<(), ProtocolError> {
        match self {
            ...
            Self::PtyReq {
                ...
                writer.write_string_utf8("pty-req")?;
                writer.write_boolean(want_reply)?;
                writer.write_string_utf8(term)?;
                writer.write_uint32(width_chars)?;
                writer.write_uint32(height_rows)?;
                writer.write_uint32(width_pixels)?;
                writer.write_uint32(height_pixels)?;
            }
            Self::Other {
                request_type,
                want_reply,
    ...
    pub fn want_reply(self) -> bool {
        match self {
            ...
            Self::PtyReq { want_reply, .. } => want_reply,
