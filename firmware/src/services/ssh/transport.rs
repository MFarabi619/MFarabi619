use super::channel::{Channel, Pipe};
use super::codec::{ObjectHasher, ObjectWriter};
use super::error::{Error, ProtocolError};
use super::types::{self, AuthMethod, Behavior, Request, SecretKey, TransportError};
use super::wire;
...
impl<'a, T: Behavior> Transport<'a, T> {
    ...
    pub(crate) fn client_ssh_id_string(&self) -> &str {
        ...
        super::unwrap_unreachable(core::str::from_utf8(slice).ok())
    }
    ...
    pub(crate) fn channel_request(&self) -> Request<T::Command> {
        super::unwrap_unreachable(self.request.clone())
    }

    pub(crate) fn channel_user(&self) -> T::User {
        super::unwrap_unreachable(self.current_user.clone())
    }
    ...
    fn channel_state(&mut self) -> &mut ChannelState {
        super::unwrap_unreachable(self.active_channel.as_mut())
    }
    ...
    async fn poll_client(&mut self) -> Result<Option<usize>, TransportError<T>> {
        ...
        match message {
                    ...
                    match user_auth_method {
                        ...
                        AuthMethod::PublicKey(types::PublicKey::Ed25519 { public_key }) => {
                            ...
                            } = auth_method
                            {
                                ...
                                writer.write_string(
                                    &super::unwrap_unreachable(self.curr_keys.as_ref()).session_id,
                                )?;
            ...
            // PTY request — accept it and store terminal dimensions
            wire::Message::ChannelRequest {
                recipient_channel,
                request: wire::Request::PtyReq { want_reply, width_chars, height_rows, .. },
            } if self.authenticated => {
                if let Some(channel_state) = &mut self.active_channel {
                    if channel_state.rx_channel_id == recipient_channel {
                        self.behavior.on_pty_request(width_chars, height_rows);
                        if want_reply {
                            let sender_channel = channel_state.tx_channel_id;
                            self.send(wire::Message::ChannelSuccess {
                                recipient_channel: sender_channel,
