use core::sync::atomic::Ordering;
use embassy_time::{Duration, Timer};
use log_04::info;
use static_cell::StaticCell;
use zephyr::{embassy::Executor, raw::*};

unsafe extern "C" {
    fn wifi_pre_start();
    fn sdcard_mount_filesystem() -> core::ffi::c_int;
    fn schedule_deep_sleep();
    fn prompt_init(shell: *const core::ffi::c_void) -> bool;
    fn prompt_print_motd(shell: *const core::ffi::c_void, ip: *const u8);
    fn mqtt_helper_get_epoch_seconds() -> i64;
}

fn capture_boot_epoch_once() {
    if crate::services::diagnostics::BOOT_EPOCH_SECONDS.load(Ordering::Relaxed) > 0 {
        return;
    }
    let epoch_now = unsafe { mqtt_helper_get_epoch_seconds() };
    if epoch_now <= 0 {
        return;
    }
    let uptime_seconds = unsafe { k_uptime_get() / 1000 };
    let boot_epoch = (epoch_now - uptime_seconds) as u32;
    crate::services::diagnostics::BOOT_EPOCH_SECONDS.store(boot_epoch, Ordering::Relaxed);
}

const MQTT_RECONNECT_DELAY_SECONDS: u64 = 10;
const MQTT_WIFI_WAIT_SECONDS: u64 = 5;
const SCD41_INIT_MAX_ATTEMPTS: u32 = 5;

#[embassy_executor::task]
async fn scd41_init_task() {
    for attempt in 1..=SCD41_INIT_MAX_ATTEMPTS {
        Timer::after(Duration::from_secs(attempt as u64)).await;
        match crate::sensors::init_co2() {
            Ok(()) => {
                info!("SCD41 init succeeded on attempt {}", attempt);
                return;
            }
            Err(error) => {
                info!("SCD41 init attempt {} failed: {}", attempt, error);
            }
        }
    }
    info!("SCD41 init giving up after {} attempts", SCD41_INIT_MAX_ATTEMPTS);
}

#[embassy_executor::task]
async fn mqtt_task() {
    Timer::after(Duration::from_secs(MQTT_WIFI_WAIT_SECONDS)).await;

    if let Err(error) = crate::services::mqtt::init() {
        info!("MQTT init failed: {:?}", error);
        return;
    }

    if !crate::services::mqtt::is_configured() {
        info!("MQTT broker not configured, skipping MQTT task");
        return;
    }

    loop {
        while !crate::networking::wifi::is_connected() {
            Timer::after(Duration::from_secs(MQTT_WIFI_WAIT_SECONDS)).await;
        }

        if let Err(error) = crate::services::mqtt::connect() {
            info!("MQTT connect failed before TCP/CONNECT: {:?}", error);
            Timer::after(Duration::from_secs(MQTT_RECONNECT_DELAY_SECONDS)).await;
            continue;
        }

        for _ in 0..50 {
            let _ = crate::services::mqtt::poll(100);
            if crate::services::mqtt::is_connected() {
                break;
            }
            Timer::after(Duration::from_millis(100)).await;
        }

        if !crate::services::mqtt::is_connected() {
            info!("MQTT no CONNACK in 5s - broker silent or rejecting handshake");
            let _ = crate::services::mqtt::disconnect();
            Timer::after(Duration::from_secs(MQTT_RECONNECT_DELAY_SECONDS)).await;
            continue;
        }

        crate::services::mqtt::note_connection_state(true);
        crate::services::home_assistant::publish_discovery_configs().await;
        crate::services::publish::publish_config_state();
        crate::services::publish::publish_firmware_info();
        crate::services::publish::publish_update_state();

        while crate::services::mqtt::is_connected() {
            capture_boot_epoch_once();
            crate::services::publish::publish_all();

            if crate::services::mqtt::deep_sleep_enabled() {
                let _ = crate::services::mqtt::disconnect();
                unsafe { schedule_deep_sleep() };
                Timer::after(Duration::from_secs(MQTT_RECONNECT_DELAY_SECONDS)).await;
                break;
            }

            let interval = crate::services::mqtt::publish_interval() as u64;
            let deadline = embassy_time::Instant::now() + Duration::from_secs(interval);
            while embassy_time::Instant::now() < deadline && crate::services::mqtt::is_connected() {
                let keepalive_ms = crate::services::mqtt::keepalive_time_left();
                let poll_timeout = keepalive_ms.min(500);
                let _ = crate::services::mqtt::poll(poll_timeout);
                crate::services::mqtt::note_connection_state(crate::services::mqtt::is_connected());

                if let Some((topic, payload)) = crate::services::mqtt::get_incoming_command() {
                    crate::services::commands::handle_command(topic, payload).await;
                }

                Timer::after(Duration::from_millis(100)).await;
            }
        }

        crate::services::mqtt::note_connection_state(false);
        info!("MQTT connection lost, reconnecting");
        Timer::after(Duration::from_secs(MQTT_RECONNECT_DELAY_SECONDS)).await;
    }
}

static EXECUTOR: StaticCell<Executor> = StaticCell::new();

#[unsafe(no_mangle)]
extern "C" fn rust_main() {
    unsafe {
        wifi_pre_start();
    }

    unsafe {
        zephyr::set_logger().unwrap();
    }
    info!("Microvisor starting");

    let boot_count = crate::services::diagnostics::increment_boot_count();
    info!("Boot count: {}", boot_count);

    unsafe {
        sdcard_mount_filesystem();
        // schedule_deep_sleep();
    }

    crate::programs::neopixel::init();
    crate::programs::motor::init();
    crate::networking::wifi::init();

    unsafe {
        let ret = http_server_start();
        if ret == 0 {
            info!("HTTP server started");
        } else {
            info!("HTTP server start failed: {}", ret);
        }
    }

    let executor = EXECUTOR.init(Executor::new());
    executor.run(|spawner: embassy_executor::Spawner| {
        spawner.spawn(crate::networking::wifi::task().unwrap());
        spawner.spawn(mqtt_task().unwrap());
        spawner.spawn(scd41_init_task().unwrap());
    })
}
