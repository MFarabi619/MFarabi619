...
#+begin_src bash
sudo pkg install yazi git {ba,z}sh bat lazygit lf terminfo-db dunst mpv direnv zellij markdown markdownfmt jetbrains-mono nerd-fonts noto-emoji npm vips docker docker-compose k9s
#+end_src
...
#+begin_src bash
sudo pkg install s-tui stress smartmontools gnupg {sy,sys,nif,bat}mon
#+end_src

#+begin_src bash
sudo pkg install procs btop fastfetch cmatrix cowsay asciiquarium figlet {lol,nyan}cat rgb-tui
#+end_src
...
#+begin_src bash
sudo pkg install go wget tree-{sitter,sitter-{cli,graph,grammars}} vim neovim py311-pynvim emacs-devel ffmpegthumbnailer coreutils cmake poppler {un,7-}zip aspell en-aspell aspell-ispell
#+end_src
...
#+begin_src bash
sudo pkg install dbus kitty vlc texlive-full webcamd gimp-jxl-plugin mousepad \
    swww sddm sddm-freebsd-black-theme \
    xdg-{terminal-exec,desktop-portal,desktop-portal-hyprland} \
    rofi wlogout dolphin pavucontrol wl-clipboard swaylock-effects \
    pwcview fontpreview epdfview glcapsviewer pango cairo pipewire grim slurp \
    hypr{land,land-qt-support,graphics,lang,utils,wayland-scanner,sunset,idle,lock,paper,picker} \
    way{land,bar} wayland-{logout,protocols,utils} qt6-{wayland,imageformats} xwayland xwayland-{satellite,run} \
    cava nwg-{look,dock-hyprland} wifimgr byobu nsxiv pamixer pa-applet wmwifi aimage qimageblitz xloadimage terminal-image-viewer
#+end_src
...
#+begin_src bash
sudo pkg install yt-dlp icu sniffnet libvirt libvirt-dbus virtboard virt-manager virt-viewer pkg_tree diskimage-tools kubectl minikube masscan ezjail gstreamer1 openscad-devel libudisks wireplumber security/portacl-rc virtualbox-ose qt5-sqldrivers-mysql qt5-sqldrivers-odbc qt5-sqldrivers-pgsql qt5-sqldrivers-sqlite2 qt5-sqldrivers-sqlite3 qt5-sqldrivers-tds netpbm
#+end_src
...
*** Other
#+begin_src bash
sudo pkg install framework-system pkg_search podman aichat ollama python {h,b,z,j,nats-,sn,usb}top desktop-installer ntopng mkdesktop portopscli xdesktopwaves treetop toppler radeon top ghostscript netpbm redis wl-gammarelay-rs wl-mkbptr wl-screenrec networkmgr firefox qmk_hid realtek-re-kmod realtek-re-kmod198 linux-steam-utils win-proton libc6-shim languageclient-neovim firefox linux-bitwarden-cli mixertui audio/moc scrot wf-recorder
#+end_src
