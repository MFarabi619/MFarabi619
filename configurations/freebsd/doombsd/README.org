...
#+begin_src sh
sudo pkg install bash zsh git lazygit git-delta direnv television ripgrep eza yazi fd docker-machine neovim emacs-devel
#+end_src
...
#+begin_src zsh
sudo pkg install ncurses tree fzf fd-find ripgrep-all cmake markdown markdownfmt tidy-html5 zig zls go gopls uv python lua51 stylua lua51-luarocks rubygem-neovim hs-ShellCheck gnuplot wget grpcurl socat websocat jq libgraphqlparser tree-{sitter,sitter-{cli,graph,grammars}} shfmt vim py311-{grip,pynvim,sqlfluff,grep-ast} plantuml ffmpeg ffmpegthumbnailer coreutils poppler {un,7-}zip aspell aspell-ispell llvm
#+end_src
...
#+begin_src zsh
sudo sysrc -f /boot/loader.conf fusefs_load="YES"
...
sudo sysrc -f /boot/loader.conf virtio_load="YES"
sudo sysrc -f /boot/loader.conf virtio_pci_load="YES"
...
#+begin_src zsh
sudo pkg install mise xdotool gdk-pixbuf2 libappindicator grafana
#+end_src
