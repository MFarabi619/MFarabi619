...
#+begin_src bash
sudo pkg install dbus kitty vlc dunst mpv texlive-full firefox webcamd gimp-jxl-plugin mousepad \
...
#+begin_src bash
sudo pkg install docker docker-compose minikube kubectl k9s yt-dlp icu sniffnet libvirt libvirt-dbus virt{ualbox-ose,board,-manager,-viewer} diskimage-tools masscan ezjail gstreamer1 openscad-devel libudisks wireplumber security/portacl-rc qt5-sqldrivers-{mysql,odbc,pgsql,sqlite2,sqlite3,tds} netpbm
#+end_src
