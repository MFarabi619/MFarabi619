Work in Progress.

=man tuning=

#+begin_src sh
mkdir release-media
bsdtar -xf FreeBSD-15.0-RELEASE-amd64-dvd1.iso -C release-media
#+end_src

* Connect to network

Check =/etc/rc.conf= to see if ifconfig_wlan0 = "WPA DHCP"

#+begin_src sh
# add 127.0.0.1 as first entry for DNS
doas bsdconfig networking
#+end_src

#+begin_src sh
doas service netif restart
#+end_src

#+begin_src sh
ifconfig wlan0
#+end_src

#+begin_src sh
ifconfig wlan0 up
#+end_src

#+begin_src sh
doas ifconfig wlan0 scan
#+end_src

#+begin_src sh
doas dhclient wlan0
#+end_src

#+begin_src conf :tangle /etc/wpa_supplicant.conf :tangle no
ctrl_interface=/var/run/wpa_supplicant

ap_scan=1
fast_reauth=1
# update_config=1

network={
  ssid="openws"
  psk="**"
}
#+end_src

* Enable =doas=

#+begin_src sh
pkg install doas
#+end_src

#+begin_src conf :tangle /usr/local/etc/doas.conf :tangle no
permit :wheel
permit nopass :wheel
permit mfarabi as root
permit keepenv mfarabi as root
#+end_src

* Set up shell
#+begin_src sh
pkg update && pkg upgrade -y
#+end_src

#+begin_src sh
doas pkg install bash zsh git lazygit git-delta direnv television ripgrep eza yazi fd docker-machine neovim emacs-devel
#+end_src

#+begin_src sh
git config --global user.email "mfarabi619@gmail.com"
git config --global user.name "Mumtahin Farabi"
#+end_src

[[https://ohmyz.sh/][Oh My Zsh]]
#+begin_src zsh
sh -c "$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)"
#+end_src

[[https://github.com/romkatv/powerlevel10k][PowerLevel10K]]
#+begin_src zsh
git clone https://github.com/romkatv/powerlevel10k.git $ZSH_CUSTOM/themes/powerlevel10k
#+end_src

[[https://github.com/chrissicool/zsh-256color][zsh-256color]]
#+begin_src zsh
git clone https://github.com/chrissicool/zsh-256color ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-256color
#+end_src

[[https://github.com/zsh-users/zsh-autosuggestions][zsh-autosuggestions]]
#+begin_src zsh
git clone https://github.com/zsh-users/zsh-autosuggestions ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-autosuggestions
#+end_src

[[https://github.com/zsh-users/zsh-syntax-highlighting][zsh-syntax-highlighting]]
#+begin_src zsh
git clone https://github.com/zsh-users/zsh-syntax-highlighting ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-syntax-highlighting
#+end_src

#+begin_src zsh
git clone https://github.com/MFarabi619/MFarabi619
#+end_src

#+begin_src zsh :tangle ~/.zshenv
source /home/mfarabi/MFarabi619/configurations/freebsd/.zshenv
#+end_src

#+begin_src zsh :tangle ~/.zshrc
source /home/mfarabi/MFarabi619/configurations/freebsd/.zshrc
#+end_src

#+begin_src zsh
mkdir ~/.config/hypr;
ln -nfs /home/mfarabi/MFarabi619/modules/home/programs/hyprland/hypr /home/mfarabi/.config/hypr;
#+end_src

[[https://github.com/doomemacs/doomemacs?tab=readme-ov-file#install][Doom Emacs]]
#+begin_src zsh
git clone --depth 1 https://github.com/doomemacs/doomemacs ~/.config/emacs;
~/.config/emacs/bin/doom install;
#+end_src

#+begin_src zsh
doas pkg install nix curl bat zellij jetbrains-mono nerd-fonts noto-emoji npm vips lf terminfo-db
#+end_src

#+begin_src zsh
fc-cache -fv
#+end_src

#+begin_src zsh
doas /usr/local/share/nix/add-nixbld-users 12
#+end_src

#+begin_src zsh
doas mkdir /etc/nix
doas cp ~/MFarabi619/configurations/freebsd/nix.conf /etc/nix/
doas sysrc nix_daemon_enable="YES"
#+end_src

#+begin_src zsh
ya pkg add yazi-rs/plugins:{git,mime-ext,smart-enter,smart-filter,smart-paste,chmod,mount,toggle-pane,vcs-files,zoom}
#+end_src

#+begin_src zsh
doas pkg install tailscale systeroid gh ttyd sysutils/lazyssh
#+end_src

#+begin_src zsh
doas sysrc tailscaled_enable="YES"
#+end_src

#+begin_src conf :tangle /var/unbound/conf.d/tailscale.conf :tangle no
server:
    private-domain: "taila4d019.ts.net"
    domain-insecure: "taila4d019.ts.net"
    local-zone: "taila4d019.ts.net." nodefault

forward-zone:
    name: "taila4d019.ts.net"
    forward-addr: 100.100.100.100
#+end_src

#+begin_src zsh
doas service local_unbound restart
doas local-unbound-control list_forwards
drill @127.0.0.1 framework-desktop.taila4d019.ts.net
drill @100.100.100.100 framework-desktop.taila4d019.ts.net
#+end_src

#+begin_src conf :tangle ~/.ssh/config
Host *
  CheckHostIP yes
  ControlPersist 10m
  AddKeysToAgent yes
  ControlMaster auto
  ControlPath ~/.ssh/cm-%C
  ExitOnForwardFailure yes
  SetEnv TERM=xterm-256color

Host framework-desktop rpi* u*
  HostName %h.taila4d019.ts.net
#+end_src

#+begin_src zsh
doas tailscale set --operator=$USER
doas tailscale up --ssh --accept-routes --accept-dns=false
#+end_src

#+begin_src zsh
doas pkg install btop fastfetch cmatrix cowsay asciiquarium figlet {lol,nyan}cat rgb-tui
#+end_src

#+begin_src zsh
doas pkg install termshark
doas chgrp network /dev/bpf*
doas chmod g+r /dev/bpf*
doas chmod g+w /dev/bpf*
#+end_src

Add to =/etc/devfs.conf=
#+begin_src zsh
doas tee -a /etc/devfs.conf <<'EOF'
own  bpf* root:network
perm bpf* 0660
EOF
#+end_src

* Set up editors

#+begin_src zsh
doas pkg install ncurses tree fzf fd-find ripgrep-all cmake markdown markdownfmt tidy-html5 zig zls go gopls uv python lua51 stylua lua51-luarocks rubygem-neovim hs-ShellCheck gnuplot wget grpcurl socat websocat jq libgraphqlparser tree-{sitter,sitter-{cli,graph,grammars}} shfmt vim py311-{grip,pynvim,sqlfluff,grep-ast} plantuml ffmpeg ffmpegthumbnailer coreutils poppler {un,7-}zip aspell aspell-ispell llvm
#+end_src

#+begin_src zsh
go install github.com/jessfraz/dockfmt@latest
go install github.com/fatih/gomodifytags@latest
go install github.com/cweill/gotests/gotests@latest
go install github.com/x-motemen/gore/cmd/gore@latest
go install github.com/jorgerojas26/lazysql@latest
go install -ldflags "-s -w" github.com/tristanisham/zvm@latest
#+end_src

#+begin_src zsh
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh
#+end_src

#+begin_src zsh
cargo install cargo-binstall
cargo binstall crates-tui
#+end_src

* Set up FreeBSD Hyprland

1. Add user to =video= group
  #+begin_src zsh
  doas pw groupmod video -m mfarabi
  groups mfarabi
  #+end_src

#+begin_src zsh
doas pkg install xclip sensors dbus kitty vlc dunst mpv firefox webcamd gimp-jxl-plugin mousepad \
    Kvantum swww sddm sddm-freebsd-black-theme \
    xdg-{terminal-exec,desktop-portal,desktop-portal-{hyprland,wlr,gtk}} \
    rofi wlogout dolphin swaylock-effects \
    {fontpre,epdf,pwc}view glcapsviewer cairo pipewire grim slurp libnotify \
    hypr{land,land-qt-support,graphics,lang,utils,wayland-scanner,sunset,idle,lock,paper,picker} \
    way{land,bar} wayland-{logout,protocols,utils} qt6-{wayland,imageformats} xwayland xwayland-{satellite,run} wl-{clipboard,screenrec,gammarelay-rs} \
    cava nwg-{look,dock-hyprland} {network,wifi}mgr byobu nsxiv pa{ngo,vucontrol,mixer,-applet} wmwifi aimage qimageblitz xloadimage terminal-image-viewer
#+end_src

#+begin_src zsh
doas sysrc webcamd_enable=YES
doas service devd restart
doas pw groupmod webcamd -m mfarabi
doas kldload cuse fusefs
doas sysrc kld_list+=cuse
doas sysrc kld_list+=coretemp
doas webcamd -l
#+end_src

 # imlib2-jxl xwaylandvideobridge hyprland-qtutils

#+begin_src zsh
doas pkg install bsd{-splash-changer,ebfetch,info,sensors} pwcbsd wmbsdbatt openconnect-freebsd-daemon viewglob sysctl{tui,view}
#+end_src

Add to =/boot/loader.conf=
#+begin_src zsh
doas sysrc -f /boot/loader.conf fusefs_load="YES"
doas sysrc -f /boot/loader.conf sysctlinfo_load="YES"
# hw.nvidiadrm.modeset="1" # if using nvidia-drm-kmod
doas sysrc -f /boot/loader.conf sysctlbyname_improved_load="YES"
#+end_src

Add to =/etc/sysctl.conf=
#+begin_src zsh
doas sysctl vfs.usermount=1
echo 'vfs.usermount=1' | doas tee -a /etc/sysctl.conf
# now you can mount directories from a remote machine to local like this
# sshfs user@remote:path ./ -o reconnect
# sshfs mfarabi@10.0.0.198:/Users/mfarabi/workspace/HyDE ./HyDE -o reconnect
#+end_src

Add the following entry to =/etc/gettytab=
#+begin_src zsh
doas pkg install ly
echo 'Ly:\
	:lo=/usr/local/bin/ly:\
	:al=root:' | doas tee -a /etc/gettytab

tail -n 5 /etc/gettytab
#+end_src

Modify the command field of the ttyv1 terminal entry in =/etc/ttys=

#+begin_src zsh
doas sed -i '' '/^ttyv1[[:space:]]/{
s/^/#/
a\
ttyv1   "/usr/libexec/getty Ly"     xterm onifexists secure
}' /etc/ttys

grep ttyv1 /etc/ttys
#+end_src

Make sure that "tty" is set appropriately in the =/usr/local/etc/ly/config.ini= file if
you decide to configure a different TTY for Ly instead.

2. Install packages

#+begin_src zsh
doas pkg install texlive-full
#+end_src

#+begin_src zsh
doas pkg install procs ncdu dust duf doggo s-tui stress smartmontools gnupg {sys,nif,bat,b}mon docker docker-{compose,machine} minikube kubectl k9s yt-dlp icu sniffnet libvirt libvirt-dbus virt{ualbox-ose,board,-manager,-viewer} diskimage-tools masscan ezjail gstreamer1 openscad-devel libudisks wireplumber security/portacl-rc qt5-sqldrivers-{mysql,odbc,pgsql,sqlite2,sqlite3,tds} netpbm
#+end_src

#+begin_src zsh
doas sysrc smartd_enable="YES"
doas sysrc libvirtd_enable="YES"
doas sysrc vboxnet_enable="YES"
#+end_src

Add to =/boot/loader.conf=
#+begin_src zsh
doas sysrc -f /boot/loader.conf vboxdrv_load="YES"
#+end_src

#+begin_src zsh
doas pw groupmod vboxusers -m $USER;
doas pw groupmod operator -m $USER;
#+end_src

Reboot machine, then:
#+begin_src zsh
doas chown root:vboxusers /dev/vboxnetctl
doas chmod 0660 /dev/vboxnetctl
#+end_src

Add to =/etc/devfs.conf=
#+begin_src zsh
own     vboxnetctl root:vboxusers
perm    vboxnetctl 0660
#+end_src

Add the following to =/etc/devfs.rules= (create if it doesn't exist):
#+begin_src zsh
[system=10]
add path 'usb/*' mode 0660 group operator
#+end_src

Add the following lines in =/etc/fstab=:

#+begin_src zsh
fdesc	/dev/fd		fdescfs		rw	0	0
proc	/proc		procfs		rw	0	0
#+end_src

#+begin_src zsh
doas sysrc devfs_system_ruleset="system"
#+end_src

https://wiki.freebsd.org/Docker
Create or edit =/usr/local/etc/vbox/networks.conf= and either make sure it allows the 192.168.99.1/24 network range used by docker-machine:

#+begin_src zsh
doas mkdir /usr/local/etc/vbox
echo "* 192.168.99.1/24" | doas tee -a /usr/local/etc/vbox/networks.conf >/dev/null
#+end_src

#+begin_src zsh
doas cp /usr/local/share/examples/libvirt/networks/default.xml /usr/local/etc/libvirt/qemu/networks
#+END_src

#+begin_src zsh
doas ln -s ../default.xml /usr/local/etc/libvirt/qemu/networks/autostart/default.xml
#+end_src

#+begin_src zsh
docker-machine create default
#+end_src

#+begin_src zsh
go install github.com/jesseduffield/lazydocker@latest
#+end_src

#+begin_src zsh
eval $(docker-machine env)
#+end_src

** Set up GPU
***  Verify display device
  #+begin_src zsh
  pciconf -lv | grep -B3 display
  #+end_src

  #+begin_src zsh
  # Example output
  vgapci0@pci0:0:2:0: class=0x030000 card=...
  vendor = 'Intel Corporation'
  device = 'HD Graphics 620'
  #+end_src

***  Install graphics driver

[[https://docs.freebsd.org/en/books/handbook/x11/#x-graphic-card-drivers][Chapter 5.2. Graphics Drivers - FreeBSD Handbook]]

| *Type*                       | *License*     | *Module*                               | *Port*                                        |
|----------------------------+-------------+--------------------------------------+---------------------------------------------|
| Intel®                     | Open Source | i915kms                              | graphics/drm-kmod                           |
| AMD®                       | Open Source | amdgpu/radeonkms                     | graphics/drm-kmod                           |
| NVIDIA®                    | Proprietary | nvidia-drm/nvidia-modeset/nvidia     | graphics/nvidia-drm-kmod, x11/nvidia-driver |
| System Console Framebuffer | Open Source | scfb                                 | x11-drivers/xf86-video-scfb                 |
| VESA BIOS Extension        | Open Source | vesa                                 | x11-drivers/xf86-video-vesa                 |
| VirtualBox®                | Open Source | vboxvideo                            | emulators/virtualbox-ose-additions          |
| VMware®                    | Open Source | vmwgfx                               | x11-drivers/xf86-video-vmware               |

#+begin_src zsh
doas pkg install drm-kmod # Intel & AMD
#+end_src

#+begin_src zsh
doas pw groupmod seatd -m mfarabi
#+end_src

#+begin_src zsh
doas sysrc seatd_enable="YES"    # let non-root graphics car users manage access to shared system devices
doas sysrc dbus_enable="YES"     # message bus and hardware abstraction
doas sysrc kld_list+=amdgpu
# doas sysrc kld_list+=i915kms
#+end_src

Add to =/etc/rc.conf=.
#+begin_src
# ========= SERVICES/DAEMONS ===========
sshd_enable="YES"     # ssh
# sddm_enable="YES"     # login manager
smartd_enable="YES"   # disk
moused_enable="YES"   # mouse
libvirtd_enable="YES" # virtualization

# ========== KERNEL MODULES ============
kld_list="i915kms"  # kernel loadable modules - graphics

# ========== MESSAGES BUFFER ============
# net.local.stream.recvspace=65536
# net.local.stream.sendspace=65536
#+end_src


doas sysrc -f /boot/loader.conf virtio_load="YES"
doas sysrc -f /boot/loader.conf virtio_pci_load="YES"

Add to =/boot/loader.conf=.
#+begin_src zsh
kern.vty=vt
#+end_src

Support for 7zip and rar archives requires the appropriate executables.
If ark has been packaged without options 7ZIP or RAR, you can install
archivers/7-zip (for 7ZIP) or archivers/unrar and archivers/rar (for RAR)
to allow ark to handle those archives.
=====
Message from bluez-firmware-1.2:

--
NOTICE
======

You need to have the ubtbcmfw(8) and ng_ubt(8) kernel modules
in our kernel. This can be accomplished by adding the following
lines to /boot/loader.conf:

 ubtbcmfw_load=yes
 ng_ubt_load=yes

#+begin_src zsh
mkdir ~/.local/share/bin
ln -sfn /home/mfarabi/MFarabi619/configurations/freebsd/.local/share/bin/* ~/.local/share/bin/

ln -sfn /home/mfarabi/MFarabi619/configurations/freebsd/.config/!(nvim,go,lazydocker,lazygit,yazi) /home/mfarabi/.config/
#+end_src

#+begin_src toml :tangle /home/mfarabi/.config/yazi/yazi.toml
[mgr]
ratio = [1, 4, 3]
show_hidden = true
show_symlink = true
sort_dir_first = true
#+end_src

#+begin_src toml :tangle /home/mfarabi/.config/yazi/theme.toml
[completion.active]
bg = "#665c54"
fg = "#d3869b"

[completion.border]
fg = "#83a598"

[completion.inactive]
fg = "#d5c4a1"

[[filetype.rules]]
fg = "#8ec07c"
mime = "image/*"

[[filetype.rules]]
fg = "#fabd2f"
mime = "video/*"

[[filetype.rules]]
fg = "#fabd2f"
mime = "audio/*"

[[filetype.rules]]
fg = "#d3869b"
mime = "application/zip"

[[filetype.rules]]
fg = "#d3869b"
mime = "application/gzip"

[[filetype.rules]]
fg = "#d3869b"
mime = "application/tar"

[[filetype.rules]]
fg = "#d3869b"
mime = "application/bzip"

[[filetype.rules]]
fg = "#d3869b"
mime = "application/bzip2"

[[filetype.rules]]
fg = "#d3869b"
mime = "application/7z-compressed"

[[filetype.rules]]
fg = "#d3869b"
mime = "application/rar"

[[filetype.rules]]
fg = "#d3869b"
mime = "application/xz"

[[filetype.rules]]
fg = "#b8bb26"
mime = "application/doc"

[[filetype.rules]]
fg = "#b8bb26"
mime = "application/pdf"

[[filetype.rules]]
fg = "#b8bb26"
mime = "application/rtf"

[[filetype.rules]]
fg = "#b8bb26"
mime = "application/vnd.*"

[[filetype.rules]]
bold = true
fg = "#83a598"
url = "*/"

[[filetype.rules]]
fg = "#d5c4a1"
mime = "*"

[help.desc]
fg = "#d5c4a1"

[help.footer]
fg = "#d5c4a1"

[help.hovered]
bg = "#665c54"
fg = "#d5c4a1"

[help.on]
fg = "#d3869b"

[help.run]
fg = "#8ec07c"

[indicator.current]
bg = "#504945"
bold = true

[indicator.preview]
bg = "#504945"
bold = true

[input.border]
fg = "#83a598"

[input.selected]
bg = "#665c54"

[input.title]
fg = "#d5c4a1"

[input.value]
fg = "#d5c4a1"

# [mgr]
# syntect_theme = "/nix/store/3fwq2aqcfxjwwq9h6fflwh5x7070zs3x-base16-gruvbox-dark-hard.tmTheme"

[mgr.border_style]
fg = "#bdae93"

[mgr.count_copied]
bg = "#b8bb26"
fg = "#1d2021"

[mgr.count_cut]
bg = "#fb4934"
fg = "#1d2021"

[mgr.count_selected]
bg = "#fabd2f"
fg = "#1d2021"

[mgr.cwd]
fg = "#8ec07c"

[mgr.find_keyword]
bold = true
fg = "#b8bb26"

[mgr.find_position]
fg = "#d3869b"

[mgr.marker_copied]
bg = "#b8bb26"
fg = "#b8bb26"

[mgr.marker_cut]
bg = "#fb4934"
fg = "#fb4934"

[mgr.marker_selected]
bg = "#fabd2f"
fg = "#fabd2f"

[mode.normal_alt]
bg = "#1d2021"
fg = "#83a598"

[mode.normal_main]
bg = "#83a598"
bold = true
fg = "#1d2021"

[mode.select_alt]
bg = "#1d2021"
fg = "#b8bb26"

[mode.select_main]
bg = "#b8bb26"
bold = true
fg = "#1d2021"

[mode.unset_alt]
bg = "#1d2021"
fg = "#d65d0e"

[mode.unset_main]
bg = "#d65d0e"
bold = true
fg = "#1d2021"

[pick.active]
fg = "#d3869b"

[pick.border]
fg = "#83a598"

[pick.inactive]
fg = "#d5c4a1"

[status.perm_exec]
fg = "#b8bb26"

[status.perm_read]
fg = "#fabd2f"

[status.perm_sep]
fg = "#8ec07c"

[status.perm_type]
fg = "#83a598"

[status.perm_write]
fg = "#fb4934"

[status.progress_error]
bg = "#1d2021"
fg = "#fb4934"

[status.progress_label]
bg = "#1d2021"
fg = "#d5c4a1"

[status.progress_normal]
bg = "#1d2021"
fg = "#d5c4a1"

[tabs.active]
bg = "#83a598"
bold = true
fg = "#1d2021"

[tabs.inactive]
bg = "#3c3836"
fg = "#83a598"

[tasks.border]
fg = "#83a598"

[tasks.hovered]
bg = "#665c54"
fg = "#d5c4a1"

[tasks.title]
fg = "#d5c4a1"

[which.cand]
fg = "#8ec07c"

[which.desc]
fg = "#d5c4a1"

[which.mask]
bg = "#504945"

[which.rest]
fg = "#d65d0e"

[which.separator_style]
fg = "#bdae93"
#+end_src

Reboot system.
#+begin_src zsh
reboot
#+end_src

Verify DRM driver.
#+begin_src zsh
dmesg | grep drm; ls /dev/dri;
#+end_src

Expected:
#+begin_src zsh
[drm] Initialized i915 ...
/dev/dri/card0
/dev/dri/renderD128
#+end_src

Start Hyprland.
#+begin_src zsh
# seatd-launch hyprland
hyprland
#+end_src

#+begin_src zsh
swww img ~/MFarabi619/configurations/freebsd/wallpapers/freebsd-gruvbox-rainbow-dark.webp
#+end_src

* Other

#+begin_src sh
doas sysrc linux_enable="YES"
#+end_src

#+begin_src sh
doas service linux start
#+end_src

#+begin_src sh
doas pkg install debootstrap
#+end_src

#+begin_src sh
doas sysrc kld_list+="linux64 fdescfs linprocfs linsysfs tmpfs"
doas kldload linux64 fdescfs linprocfs linsysfs tmpfs
#+end_src

#+begin_src sh
doas debootstrap jammy /compat/ubuntu
#+end_src

#+begin_src
devfs           /compat/ubuntu/dev      devfs           rw,late                      0       0
tmpfs           /compat/ubuntu/dev/shm  tmpfs           rw,late,size=1g,mode=1777    0       0
fdescfs         /compat/ubuntu/dev/fd   fdescfs         rw,late,linrdlnk             0       0
linprocfs       /compat/ubuntu/proc     linprocfs       rw,late                      0       0
linsysfs        /compat/ubuntu/sys      linsysfs        rw,late                      0       0
/tmp            /compat/ubuntu/tmp      nullfs          rw,late                      0       0
/home           /compat/ubuntu/home     nullfs          rw,late                      0       0
#+end_src

#+begin_src sh
doas mount -al
#+end_src

#+begin_src sh
doas chroot /compat/ubuntu /bin/bash
#+end_src

#+begin_src zsh
doas pkg install framework-{system,tool-tui} pkg_{tree,search} podman aichat tgpt ollama top {h,b,z,j,nats-,sn,usb,tree}top desktop-installer ntopng mkdesktop portopscli xdesktopwaves toppler radeon ghostscript netpbm redis qmk_hid realtek-re-{kmod,kmod198} linux-{discord,steam-utils,bitwarden-cli} win-proton libc6-shim languageclient-neovim mixertui audio/moc scrot wf-recorder
#+end_src

#+begin_src zsh
doas pkg install bandwhich mu xwininfo # for emacs-everywhere
#+end_src

#+begin_src zsh
doas kldload snd_driver
#+end_src

#+begin_src zsh
doas kldload vmm
doas ifconfig tap0 create
doas sysctl net.link.tap.up_on_open=1
doas ifconfig bridge0 create
doas ifconfig bridge0 addm ue0 addm tap0 # change `ue0` to your preferred network interface
doas ifconfig bridge0 up
doas truncate -s 100G guest.img
doas sh /usr/share/examples/bhyve/vmrun.sh -c 8 -m 48000M -t tap0 -d guest.img -i -I FreeBSD-14.3-RELEASE-amd64-bootonly.iso guestname
#+end_src

#+begin_src zsh
doas kldload nullfs
#+end_src

** Resources:
- [[https://github.com/wisonye/freebsd-handbook/blob/master/chapters/install-hyprland.org][wisonye's Personal FreeBSD Handbook - Install Hyprland]]
- [[https://lemmy.world/post/1685763][Lenny Mackners - Run Hyprland on FreeBSD]]
- [[https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=283123][i915kms regression issue]]

* Attributions

DoomBSD borrows HEAVILY from the following projects:

- [[https://github.com/HyDE-Project/HyDE][HyDE]]
- [[https://github.com/doomemacs/doomemacs][Doom Emacs - Henrik Lissner]]
  - Reading the *Introduction* was a memorable experience and instantly had me hooked
  - DoomBSD is named in reference to this project and its philosophies
- [[https://gitlab.com/Zaney/zaneyos][ZaneyOS - Tyler Kelley]]
  - Incredibly well-structured NixOS modules and a shining example of good codebase architecture that's easily navigable
- [[https://www.lazyvim.org][LazyVim]]


* Virtual Machine Setup (WIP)
** x86-64-freebsd
*** QEMU
#+begin_src sh
doas pkg install qemu
#+end_src

#+begin_src sh
if [ "$(readlink /usr/local/bin/qemu)" != "/usr/local/bin/qemu-system-x86_64" ]; then
  doas ln -sf /usr/local/bin/qemu-system-x86_64 /usr/local/bin/qemu
fi
#+end_src

#+begin_src sh
doas sysctl net.link.tap.user_open=1
doas grep -qxF "net.link.tap.user_open=1" /etc/sysctl.conf || \
echo 'net.link.tap.user_open=1' | doas tee -a /etc/sysctl.conf
doas grep -qxF "add path 'tap*' mode 0660 group operator" /etc/devfs.rules || \
printf "add path 'tap*' mode 0660 group operator\n" | doas tee -a /etc/devfs.rules
#+end_src

*** Bhyve

#+begin_src sh
doas kldload vmm
#+end_src

#+begin_src sh
doas ifconfig tap0 create
doas  sysctl net.link.tap.up_on_open=1
doas grep -qxF "net.link.tap.up_on_open=1" /etc/sysctl.conf || \
echo 'net.link.tap.up_on_open=1' | doas tee -a /etc/sysctl.conf
doas grep -qxF "add path 'tap*' mode 0660 group operator" /etc/devfs.rules || \
printf "add path 'tap*' mode 0660 group operator\n" | doas tee -a /etc/devfs.rules
#+end_src

#+begin_src sh
doas ifconfig bridge0 create
doas ifconfig bridge0 addm igb0 addm tap0
doas ifconfig bridge0 up
#+end_src

** aarch64-darwin

[[https://www.freebsd.org/where/][Download VM Image]]

#+begin_src sh
  unxz FreeBSD-15.0-RELEASE-arm64-aarch64-zfs.qcow2.xz
#+end_src

#+begin_src sh
  qemu-system-aarch64 \
    -smp 8 \
    -m 32G \
    -accel hvf \
    -cpu cortex-a72 \
    -machine virt,highmem=on \
    -device virtio-net-pci,netdev=net0 \
    -netdev user,id=net0,hostfwd=tcp::2222-:22 \
    -bios /opt/homebrew/share/qemu/edk2-aarch64-code.fd \
    -drive file=FreeBSD-15.0-RELEASE-arm64-aarch64-zfs.qcow2,if=virtio,format=qcow2 \
    -nographic
#+end_src

#+begin_src sh
  qemu-img resize FreeBSD-15.0-RELEASE-arm64-aarch64-zfs.qcow2 256G
  qemu-img info FreeBSD-15.0-RELEASE-arm64-aarch64-zfs.qcow2
#+end_src


#+begin_src sh
  df -h

  # see current layout
  gpart show
  zpool status

  # enable autoexpand (so ZFS can use larger underlying device)
  zpool set autoexpand=on zroot

  # if ZFS uses the whole disk (no partition):
  zpool online -e zroot /dev/vtbd0

  # (use gpart show to find the correct index)
  gpart show
  # if ZFS is on a partition (e.g. vtbd0p2), resize that partition first if needed:
  gpart resize -i 2 vtbd0      # resize partition index 2 to fill the disk
  zpool online -e zroot /dev/vtbd0p2

  # 1) recover the GPT backup/header if needed
  gpart recover vtbd0

  # verify layout and that GPT is no longer corrupt
  gpart show vtbd0

  # 2) resize partition index 3 (freebsd-zfs) to fill the disk
  gpart resize -i 3 vtbd0

  # confirm partition grew
  gpart show vtbd0

  # 3) ask ZFS to expand the device (use the exact member name shown by zpool status)
  zpool set autoexpand=on zroot
  zpool online -e zroot vtbd0p3

  # verify pool and fs sizes
  zpool list
  zfs list
#+end_src

** amd64

#+begin_src sh
  virsh -c qemu:///system
#+end_src

#+begin_src sh
  net-start default
  net-autostart default
  net-dumpxml default
#+end_src

#+begin_src xml
  <network>
    <name>default</name>
    <uuid>04503b12-bc89-42db-811b-7ec2c6492c84</uuid>
    <forward mode='nat'>
      <nat>
        <port start='1024' end='65535'/>
      </nat>
    </forward>
    <bridge name='virbr0' stp='on' delay='0'/>
    <mac address='52:54:00:60:fe:35'/>
    <ip address='192.168.122.1' netmask='255.255.255.0'>
      <dhcp>
        <range start='192.168.122.2' end='192.168.122.254'/>
      </dhcp>
    </ip>
  </network>
#+end_src

#+begin_src sh
  mkdir $HOME/vms
  cd $HOME/vms

  # Download VM image and unzip it
  # wget https://download.freebsd.org/releases/VM-IMAGES/15.0-RELEASE/amd64/Latest/FreeBSD-15.0-RELEASE-amd64-zfs.qcow2.xz
  wget https://download.freebsd.org/releases/VM-IMAGES/15.0-RELEASE/amd64/Latest/FreeBSD-15.0-RELEASE-amd64-BASIC-CLOUDINIT-zfs.qcow2.xz

  # Decompress but keeps the original
  # xz -dk FreeBSD-15.0-RELEASE-amd64-zfs.qcow2.xz
  xz -dk FreeBSD-15.0-RELEASE-amd64-BASIC-CLOUDINIT-zfs.qcow2.xz

  # Make the disk slightly bigger
  # qemu-img resize FreeBSD-15.0-RELEASE-amd64-zfs.qcow2 350G
  qemu-img resize FreeBSD-15.0-RELEASE-amd64-BASIC-CLOUDINIT-zfs.qcow2 350G

  doas mkdir -p /var/lib/libvirt/boot
  doas chmod 0755 /var/lib/libvirt/boot

  doas virt-install \
        --import \
        --vcpus 6 \
        --boot uefi \
        --memory 16384 \
        --graphics none \
        --os-variant freebsd15.0 \
        --name FreeBSD-15.0-RELEASE \
        --console pty,target_type=serial \
        --network network=default,model=virtio \
        --disk path=$HOME/vms/seed.iso,device=cdrom \
        --disk path=$HOME/vms/FreeBSD-15.0-RELEASE-amd64-BASIC-CLOUDINIT-zfs.qcow2,format=qcow2,bus=virtio
#+end_src

#+begin_src sh
  doas virsh list --all
  doas virsh domifaddr FreeBSD-15.0-RELEASE
  doas virsh destroy FreeBSD-15.0-RELEASE
  doas virsh undefine FreeBSD-15.0-RELEASE --nvram
  rm ~/vms/FreeBSD-15.0-RELEASE-amd64-BASIC-CLOUDINIT-zfs.qcow2
  doas cloud-localds ~/vms/seed.iso ~/MFarabi619/configurations/freebsd/doombsd/user-data.yaml
  xz -dk ~/vms/FreeBSD-15.0-RELEASE-amd64-BASIC-CLOUDINIT-zfs.qcow2.xz
  qemu-img resize ~/vms/FreeBSD-15.0-RELEASE-amd64-BASIC-CLOUDINIT-zfs.qcow2 350G
#+end_src

** Run NixOS VM Guest

#+begin_src sh
  qemu-system-x86_64 \
    -smp 3 \
    -m 8000 \
    -boot d \
    -net nic \
    -net user,hostfwd=tcp::2222-:22 \
    -drive file=nixos-25.11-x86_64.qcow2,format=qcow2 \
    -cdrom /home/mfarabi/QEMU/ISO/latest-nixos-graphical-x86_64-linux.iso
#+end_src

* Set up Caddy (WIP)

#+begin_src sh
  Proceed with this action? [y/N]: y
  [1/1] Fetching caddy-2.10.2_4.pkg: 100%   13 MiB  13.7MB/s    00:01
  Checking integrity... done (0 conflicting)
  [1/1] Installing caddy-2.10.2_4...
  ===> Creating groups
  Using existing group 'www'
  ===> Creating users
  Using existing user 'www'
  [1/1] Extracting caddy-2.10.2_4: 100%
  =====
  Message from caddy-2.10.2_4:

  --
  To enable caddy:

  - Edit /usr/local/etc/caddy/Caddyfile
    See https://caddyserver.com/docs/
  - Run 'service caddy enable'

  Note while Caddy currently defaults to running as root:wheel, it is strongly
  recommended to run the server as an unprivileged user, such as www:www --

  - Use security/portacl-rc to enable privileged port binding:

    # pkg install security/portacl-rc
    # sysrc portacl_users+=www
    # sysrc portacl_user_www_tcp="http https"
    # sysrc portacl_user_www_udp="https"
    # service portacl enable
    # service portacl start

  - Configure caddy to run as www:www

    # sysrc caddy_user=www caddy_group=www

  - Note if Caddy has been started as root previously, files in
    /var/log/caddy, /var/db/caddy, and /var/run/caddy may require their ownership
    changing manually.

  /usr/local/etc/rc.d/caddy has the following defaults:

  - Server log: /var/log/caddy/caddy.log
    (runtime messages, NOT an access.log)
  - Automatic SSL certificate storage: /var/db/caddy/data/caddy/
  - Administration endpoint: //unix/var/run/caddy/caddy.sock
  - Runs as root:wheel (this will change to www:www in the future)
#+end_src

* Set up Embedded Systems Development Toolchain (WIP)

#+begin_src
Message from sddm-freebsd-black-theme-1.3:
--
To enable this theme edit:

 /usr/local/etc/sddm.conf

 This theme use the x11-fonts/montserrat font by default. However, it
 can be changed to any desired font editing:

 /usr/local/share/sddm/themes/sddm-freebsd-black-theme/theme.conf
--
=====
Message from sddm-0.21.0.36_1:

--
SDDM lists a "user session" which needs either an .xinitrc in the user's
home directory, or as a fallback, xterm. In order to use the "user session"
feature, a ~/.xinitrc is recommended.
#+end_src

#+begin_src zsh :results drawer :exports code
curl -fsSL -o get-platformio.py https://raw.githubusercontent.com/platformio/platformio-core-installer/master/get-platformio.py
python3 get-platformio.py
#+end_src

#+begin_src zsh :results drawer :exports code
doas pkg install portshaker
doas nvim /usr/local/etc/portshaker.d/local
#+end_src

#+begin_src zsh :results drawer :exports code
#!/bin/sh
. /usr/local/share/portshaker/portshaker.subr

if [ "$1" != "--" ]; then
  err 1 "Extra arguments"
fi
shift

method="rsync"
rsync_source_path="/usr/local/ports-local"

run_portshaker_command "$@"
#+end_src

#+begin_src zsh :results drawer :exports code
doas chmod +x /usr/local/etc/portshaker.d/local
doas mkdir -p /usr/local/ports-local

doas nvim /usr/local/etc/portshaker.d/ports
#+end_src

#+begin_src zsh :results drawer :exports code
#!/bin/sh
. /usr/local/share/portshaker/portshaker.subr

if [ "$1" != "--" ]; then
  err 1 "Extra arguments"
fi
shift

method="git"
git_clone_uri="https://git.FreeBSD.org/ports.git"
git_branch="main"

run_portshaker_command "$@"
#+end_src

#+begin_src zsh :results drawer :exports code
doas chmod +x /usr/local/etc/portshaker.d/ports
#+end_src

#+begin_src zsh :results drawer :exports code
doas nvim /usr/local/etc/portshaker.conf
#+end_src

#+begin_src zsh :results drawer :exports code
mirror_base_dir="/usr/local/portshaker"
ports_trees="main"

use_zfs="no"
poudriere_ports_mountpoint="/usr/local/poudriere/ports"

main_ports_tree="/usr/ports"
main_merge_from="ports local"
#+end_src

#+begin_src zsh :results drawer :exports code
doas portshaker -v
doas portshaker -s
doas portshaker -t
#+end_src

#+begin_src zsh :results drawer :exports code
cd /usr/local/ports-local
doas git clone https://github.com/trombik/xtensa-esp32-elf.git
#+end_src

#+begin_src zsh :results drawer :exports code
doas portshaker
#+end_src

#+begin_src zsh :results drawer :exports code
doas pkg install -y arduino arduino-{core,avrdude,bsd-mk} avr{dude,-gcc}
#+end_src

#+begin_src
To allow serial port locking, add your user to the dialer group:
    pw groupmod dialer -m mfarabi
    User settings are in ~/.arduino/preferences.txt.  Edit the file
when the IDE is not running.

Setting the launcher for help files:
    launcher=firefox

Showing what the IDE is doing for build and upload commands (only
visible if you started the IDE from a console):
    build.verbose=true
    upload.verbose=true

To start the IDE:
    /usr/local/bin/arduino
#+end_src

https://wiki.freebsd.org/Arduino

#+begin_src sh
doas kldload uarduno.ko
#+end_src

#+begin_src sh
doas sysrc uarduno_load="YES"
#+end_src

#+begin_src zsh
--
Your SDL library has been built with libvgl support, which means that you
 can run almost any SDL application straight on your console (VESA 2.0
 compatible videocard is required).

 To do this you have to load the vesa kernel module or enable it in your
 kernel, and set environment variable "SDL_VIDEODRIVER=vgl".
=====
Message from pwcview-1.4.1_9:
#+end_src

#+begin_src zsh
doas pkg install stm32flash stlink openocd xtensa-esp-elf
#+end_src

FreeBSD USB NOTE:
 To allow an ordinary user to acces any of the the hotplug USB interface
 add him/her to the operator group  (pw groupmod operator -m username), then
 setup the devfs subsystem by adding these lines to the following files:

 ***/etc/devfs.rules:
 [localrules=10]
        add path 'ugen*' mode 0660 group operator
        add path 'usb/*'  mode 0660 group operator
        add path 'usb' mode 0770 group operator

 ***/etc/rc.conf:
	devfs_system_ruleset="localrules"

#+begin_src zsh
doas pkg install mise xdotool gdk-pixbuf2 libappindicator grafana
#+end_src

#+begin_src zsh
cargo install espup esp-config --features="tui"
#+end_src

#+begin_src zsh
cargo install dioxus-cli
#+end_src
