...
(defcustom pinout-font-height 2.0
  "Scale factor for the default face height in pinout buffers.
Static — no auto-fit.  Use `C-x C-=' / `C-x C--' in the buffer for
live zoom adjustments; that's the standard Emacs `text-scale' built-in
and applies cleanly on top of this base scale."
  :type 'number
  :group 'pinout)
...
(defcustom pinout-supported-boards
  '((xiao_esp32s3 . "seeed"))
  "Alist mapping board keys to their vendor under `zephyr/boards/'."
...
Each entry is (KEY . PLIST).  PLIST keys:
  ...
  :usb     -- connector spec (:type SYM :side top|bottom|left|right :offset center|start|end)
  :sides   -- alist mapping side symbol to list of pin plists
              pin plist: (:n NUM :primary STR :aliases (STR ...)
                          :pwm BOOL :touch BOOL)"
  :type 'sexp
  :group 'pinout)
...
(defun pinout--segment-role (segment)
  ...
  (cond
    ((member segment '("5V" "3V3" "VDD" "VCC" "VBUS" "3.3V-OUT"))                                        'power)
    ((member segment '("GND" "VSS"))                                                                    'ground)
    ((string= segment "RTC")                                                                            'system)
    ...
    ((string-match-p (rx bos "ADC"  digit "/A" (+ digit) eos)                                 segment) 'adc)
...
(defun pinout--draw-rect (grid rect)
  ...
  (let* ((top    (plist-get rect :chip-top))
          (bot    (plist-get rect :chip-bot))
    ...
    (pinout--grid-set grid top mid-x (pinout--cell ?┴))
    (cl-loop for c from (1+ mid-x) below right do
      (pinout--grid-set grid top c (pinout--cell ?─)))
    (pinout--grid-set grid top right (pinout--cell ?┐))
    (pinout--grid-set grid bot left  (pinout--cell ?└))
    (cl-loop for c from (1+ left) below right do
      (pinout--grid-set grid bot c (pinout--cell ?─)))
    (pinout--grid-set grid bot right (pinout--cell ?┘))
    (cl-loop for r from (1+ top) below bot do
      (pinout--grid-set grid r left  (pinout--cell ?│))
      (pinout--grid-set grid r right (pinout--cell ?│)))))
...
(defun pinout--draw-usb (grid rect usb)
  ...
  (let* ((type   (plist-get usb :type))
          ...
          (chip-left  (plist-get rect :chip-left))
          (chip-right (plist-get rect :chip-right))
    ...
    (pcase side
      ((or 'top 'bottom)
        (let* ((y (if (eq side 'top)
                    (plist-get rect :usb-row)
                    (1+ (plist-get rect :chip-bot))))
                (x (pcase offset
                     ('start  chip-left)
...
(defun pinout--draw-board-name (grid rect name)
  ...
  (let* ((left  (plist-get rect :chip-left))
          (right (plist-get rect :chip-right))
          (mid   (/ (+ left right) 2))
          (row   (plist-get rect :chip-bot))
          (label (format " %s " name))
          (x     (- mid (/ (length label) 2))))
    (pinout--grid-place grid row x label 'bold)))
...
(defun pinout--draw-legend (grid rect)
  ...
  (let* ((row      (plist-get rect :legend-row))
          (canvas-w (plist-get rect :canvas-w))
          (boxes    (mapcar (lambda (item)
    ...
    (cl-loop for (text . face) in boxes
      ...
      (when need-sep
        (pinout--grid-place grid row col sep)
        (cl-incf col (length sep)))
      (pinout--grid-place grid row col text face)
...
default dark bg to show through behind the yellow triangle glyph."
  (pcase-let ((`(,button ,left-pos ,right-pos) chip))
  ...
  (let* ((mp     (mouse-pixel-position))
    ...
    (when (and (framep frame) (numberp mx) (numberp my))
      (let* ((window (frame-selected-window frame))
              (posn   (and window (posn-at-x-y mx my window))))
        (when (and posn (windowp (posn-window posn)))
          (let ((buf (window-buffer (posn-window posn)))
                 (pos (posn-point posn)))
            (when (and (buffer-live-p buf) (numberp pos))
...
(defun pinout--render (board)
  ...
  (let* ((rect       (pinout--compute-rect board))
    ...
    (pinout--apply-face-remap)
    (insert-char ?\n (max 0 (/ (- (window-body-height) canvas-h) 2)))
...
(defun pinout--locate-board-yml (board)
  (when-let* ((vendor (alist-get board pinout-supported-boards))
...
(defvar pinout-mode-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map special-mode-map)
    (define-key map (kbd "TAB")       #'forward-button)
    (define-key map (kbd "<backtab>") #'backward-button)
    map)
  "Keymap for `pinout-mode'.")
      ...
      (lambda ()
        (dolist (buf (buffer-list))
          (when (and (buffer-live-p buf)
                  (with-current-buffer buf (derived-mode-p 'pinout-mode))
...
(defun pinout--reload-and-reset ()
  "Reload `pinout.el' and reset all defcustoms before render."
  ...
  (dolist (sym '(pinout-box-width pinout-row-spacing pinout-pin-lead-length
                  pinout-label-padding pinout-font-family pinout-font-height
                  pinout-default-board pinout-board-info-format pinout-boards))
...
to pick a board interactively."
  ...
  (let* ((key       (intern board))
          ...
          (buf-name  (format "*pinout:%s*"
                       (or (pinout--west-config-value "build" "board") board))))
    (unless data
    ...
    (let ((buffer (get-buffer-create buf-name)))
      ...
      (with-silent-modifications
        ...
        (setq pinout--current-board key)
        (pinout--render data)
        (goto-char (point-min)))
      ;; Doom-modeline colors the buffer name by `buffer-modified-p'; flag the
      ;; buffer as modified so the name renders in the same yellow as soc.el.
      (set-buffer-modified-p t))
