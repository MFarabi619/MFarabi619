...
(defcustom soc-font-height 1.0
  "Scale factor for the default face height in soc buffers."
  :type 'number :group 'soc)
...
(defun soc--draw-die (grid layout)
  ...
  (when soc-show-die-outline
    (let* ((top   (+ (plist-get layout :chip-top)   3))
            ...
            (left  (+ (plist-get layout :chip-left)  5))
            (right (- (plist-get layout :chip-right) 5))
            (face  'soc-die))
    ...
    (pcase side
      (`left
        (let* ((row (soc--pin-row layout index))
          ...
          (soc--write grid row (+ col 2) (format "%-2d" n) 'face num-face)
      ...
      (`right
        (let* ((row (soc--pin-row layout index))
          ...
          (cl-loop for i from 1 to lead do
            (soc--set grid row (+ col i) (soc--cell ?─ 'face line-face)))
          (soc--write grid row (- col 3) (format "%2d" n) 'face num-face)
...
(defun soc--apply-face-remap ()
  "Apply `soc-font-family' and `soc-font-height' via face remap."
  ...
  (let (spec)
    ...
    (when (and (numberp soc-font-height) (/= soc-font-height 1.0))
      (setq spec (append spec (list :height soc-font-height))))
    (when spec
...
window width."
  ...
  (setq-local truncate-lines               t
    ...
    revert-buffer-function #'soc--revert-buffer)
  (font-lock-mode -1))

(defun soc--revert-buffer (&rest _)
  "`revert-buffer' handler for `soc-mode'."
